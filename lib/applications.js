            ...
            if (decisionWebhook) {
                ...
                if (is_offer_available) {
                    ...
                    if (guarantors === 0) {
                        ...
                        complete.push({ "id": 2, "text": `En begränsad borgen på ${parseFloat(guarantors.toString()) * parseInt(amount.replace(/ /g, ""))} kr`, "value": `${parseFloat(guarantors.toString()) * parseInt(amount.replace(/ /g, ""))}` });
                    }
                    ...
                    offer["early_text"] = "";
                    offer["added_by"] = "Kompar API";
                    offer["offer_from_api"] = true;
        ...
        if (!checkIfApplicationExists) {
            ...
            if (offers != null && offers.length > 0) {
                ...
                for (let i = 0; i < offers.length; i++) {
                    ...
                    if (offer.api_client_id === lender_id && offer.offer_from_api) {
                        hasBeenFound = true;
                ...
                if (hasBeenFound) {
                    if (hasBeenClosed) {
                        ...
                        await db.collection(config.DB.names.client).updateOne(
                            ...
                            {
                                "$set": {
                                    "applications.$[replyapplication].offers.$[replyoffer].status": "Canceled",
                                    "applications.$[replyapplication].offers.$[replyoffer].closedDateUTC": Date.now(),
                                    "applications.$[replyapplication].offers.$[replyoffer].closedDate": config.common.toCorrectZone(Date.now()),
                                    "applications.$[replyapplication].offers.$[replyoffer].closedDateText": config.common.dateNowText(),
                                    "applications.$[replyapplication].offers.$[replyoffer].closedBy": "API"
                                },
                                "$push": {
                                    "applications.$[replyapplication].activity": {
                                        "date": config.common.toCorrectZone(Date.now()), "type": `Lender - ${res.locals.name} - offer canceled. Canceled by: Kompar API`
                                    },
                                    "applications.$[replyapplication].admin_notifications": {
                                        "id": uuidv4(),
                                        "add_date": config.common.toCorrectZone(Date.now()),
                                        "type": "Offer closed",
                                        "seen": [],
                                        "other": {
                                            "lender_id": lender_id
                                    ...
                                    "applications.$[replyapplication].filtered": {
                                        "lender_id": lender_id,
                                        "reason": reason,
                                        "decline_comment": "",
                                        "addedBy": "Kompar API"
                            ...
                            { "arrayFilters":
                                [
                                    { "replyapplication.id": application_id},
                        ...
                        await emails.admin.lender_close(db, application_id, res.locals.name, reason, test);
...
exports.signed = async (req, res, db, test) => {
    const body = req.body;
    const lender_id = res.locals.id;
    const lender_name = res.locals.name;
                ...
                if (hasBeenFound) {
                    ...
                    if (hasBeenClosed) {
                        ...
                        await db.collection(config.DB.names.client).updateOne(
                            ...
                            {   
                                "$set": {
                                    "applications.$[replyapplication].offers.$[replyOffer].status": "Signed",
                                    "applications.$[replyapplication].offers.$[replyOffer].signedDate": Date.now(),
                                    "applications.$[replyapplication].offers.$[replyOffer].signedDateUTC": config.common.toCorrectZone(Date.now()),
                                    "applications.$[replyapplication].offers.$[replyOffer].signedDateText": config.common.dateNowText(),
                                    "applications.$[replyapplication].offers.$[replyOffer].finalOffer": {
                                        "amount": chosenOffer.amount,
                                        "duration": chosenOffer.duration,
                                        "yearly_interest": chosenOffer.yearly_interest,
                                        "total": chosenOffer.total,
                                        "monthly": chosenOffer.monthly,
                                        "interest": chosenOffer.interest,
                                        "fees": chosenOffer.fees
                                    }
                                },
                                "$push": {
                                    "applications.$[replyapplication].activity": {
                                        "date": config.common.toCorrectZone(Date.now()), "type": `Lender - ${lender_name} - offer signed. Signed by: API`
                                    },
                                    "applications.$[replyapplication].admin_notifications": {
                                        "id": uuidv4(),
                                        "add_date": config.common.toCorrectZone(Date.now()),
                        ...
                        await emails.admin.lender_sign(db, application_id, lender_name, test);
