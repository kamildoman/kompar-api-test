            ...
            if (offers != null && offers != undefined && offers.length > 0) {
                ...
                if (hasBeenFound) {
                    if (hasBeenClosed) {
                        ...
                        return res.status(http.statuses.BAD_REQUEST).send({ "success": false, "message": `You have signed application ${body.application_id}.` });
                    } else {
                        await db.collection(config.DB.names.applications).updateOne(
                            { 
                                id: application_id,
            ...
            if (offers != null && offers != undefined && offers.length > 0) {
                ...
                if (hasBeenFound) {
                    if (hasBeenClosed) {
                        ...
                        return res.status(http.statuses.BAD_REQUEST).send({ "success": false, "message": `You have closed application ${body.application_id}.` });
                    } else if (hasBeenSigned) {
