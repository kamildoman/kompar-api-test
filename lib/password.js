...
exports.perform = async (req, res, db, test) => {
    ...
    try {
        ...
        await db.collection(config.DB.names.clients).findOneAndUpdate(
          { id: verifiedToken.id }, 
          { 
            $set: { 
                password: hash,
        ...
        return res.status(http.statuses.OK).send(
            {
                "message": "Password has been created. You can close that page" 
            });
    } catch(err) {
          logger.error("PASSWORD - JWT error: " + err);
          return res.status(http.statuses.NOT_FOUND).send(
            {
                "message": "There are some problems to finish creating password. Please try again or contact with it@kompar.se"
            });
    }
};