...
exports.perform = async (req, res, db) => {
    ...
    if (token == null || body.password == null) {
    ...
    try {
        ...
        if (!databaseToken) {
            ...
            return res.status(http.statuses.UNAUTHORIZED).send({ "message": "You have no permission to change password." });
        }
        console.log("dupa");
        if (databaseToken.expiresIn < Date.now()) {
            ...
            return res.status(http.statuses.UNAUTHORIZED).send({ "message": "Token is expired." });
        }
        ...
        const hash = crypto.pbkdf2Sync(body.password, salt, tokens.iterations, tokens.key_length, config.common.type.sha512).toString(config.common.type.hex) + pepper;
