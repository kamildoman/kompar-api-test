...
exports.perform = async (req, res, db, test) => {
    ...
    if (!authorization || authorization === undefined || authorization.indexOf(`${config.common.type.basic} `) === -1) {
...
exports.auth = async function(req, res, next, db, test) {
    ...
    try {
        ...
        if (!client) {
            logger.warning(`No client with this id: ${id}`);
        ...
        if (client.token.value !== jwtBearerToken) {
            logger.warning(`Wrong token value for this client. id: ${id}`);
            return res.status(http_controller.statuses.UNAUTHORIZED).send({ "success": false, "message": "Invalid credentials."});
        }

        if (client.token.expiresIn < Date.now()) {
            logger.warning(`Token is expired. id: ${id}`);
        ...
        res.locals.partner_id = client.partner_id;
        res.locals.key = client.key;
        res.locals.auth = client.auth;
        res.locals.proxy = client.proxy;
        res.locals.encryptDecision = client.encryptDecision;
        return next();
    } catch(error) {
