...
exports.flowApplication = async (res, db) => {
    const client = await db.collection(config.DB.names.client).findOne({ id: res.locals.id });
    ...
    const clients = await db.collection(config.DB.names.clients).findOne({ "applications.id": client.flow.application_id });
    if (!clients) {
