...
exports.decision = async (res, db) => {
    ...
    if (webhook) {
        ...
        const decisionBody = {
            ...
            "phone": chosenDecision.phone_number
        };
        if (res.locals.partner_id === "9") {
            decisionBody["ssn"] = "9806033768";
            decisionBody["name"] = "Test Tester"
            decisionBody["guarantors"] = [
                {
                    "ssn": "9806033768",
                    "email": chosenDecision.email,
                    "phone": chosenDecision.phone_number,
        ...
        if (res.locals.encryptDecision) {
            body2 = encryptDecision(decisionBody, webhook.key);
        } else {
            body2 = decisionBody;
        }
        const options = {
...
exports.flowStart = async (res, db) => {
    ...
    if (webhook) {
        ...
        try {
            ...
            await db.collection(config.DB.names.client).insertOne(
                {
                    "id": uuidv4(),
                    "applications":[
                        {
...
exports.flowDecision = async (res, db) => {
    const client = await db.collection(config.DB.names.clients).findOne({ id: res.locals.id });
    if (!client.flow || !client.flow.application_id) {
        logger.warning(`FLOW DECISION - no flow started. Client id: ${res.locals.id}`)
        return res.status(http.statuses.NOT_FOUND).send({ "success": false, "message": "You have to start flow first." });
    }
    const clients = await db.collection(config.DB.names.client).findOne({ "applications.id": client.flow.application_id });
    if (!clients) {
        logger.warning(`FLOW DECISION - no client. Client id: ${res.locals.id}`)
        return res.status(http.statuses.NOT_FOUND).send({ "success": false, "message": "You have to start flow first." });
    }
    const application = clients.applications[0];
    if (!application) {
        ...
        if (!webhook) {
            ...
            if (accepted) {
                ...
                body["phone"] = application.mobilePhone1;
                if (res.locals.partner_id === "9") {
                    body["ssn"] = "9806033768";
                    body["name"] = "Test Tester"
                    body["guarantors"] = [
                        {
                            "ssn": "9806033768",
                            "email": application.emailAddress,
                            "phone": application.mobilePhone1,
                            "name": "Test Tester"
                        },
                        {
                            "ssn": "7311037860",
            ...
            let body2 = null;
            if (res.locals.encryptDecision) {
                body2 = encryptDecision(body, webhook.key);
            } else {
                body2 = body;
            }
            ...
            await db.collection(config.DB.names.client).updateOne(
                ...
                {
                    $set: {
                        "applications.$[reply].sentDecisionViaWebhook": true
                    }
                },
                { "arrayFilters":
                    [
                        { 'reply.id': application.id }
                    ]
                }
            );
