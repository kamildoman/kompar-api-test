...
function encrypt2(application, key) {
    const body = {
        ...
        "mobilePhone1": application.mobilePhone1,
    ...
    if (!application) {
        ...
        if (!webhook) {
            ...
            fetch(webhook.url,
                ...
                .then(response => {
                    logger.warning(`FLOW DECISION - decision has been sent. Client id: ${res.locals.id}`)
                    return res.status(http.statuses.NOT_FOUND).send({ "success": false, "message": "Decision has been sent to your url." });
                }).catch(error => {
