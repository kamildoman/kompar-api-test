                        ...
                        if (is_clients_excluded.length === 0) {
                            try {
                                ...
                                if (webhookResponse.status === 200) {
                                    ...
                                    sents.push({ "webhook": webhook, "confirmed": false, "after_resend_not_confirmed": false });
                                }
                            } catch (error) {
    ...
    if (client_id != null && application_id != null && token != null && token === auth(test)) {
        try {
            ...
            if (application == null) {
                ...
                logger.warning("WEBHOOKS - SEND DECISION TO LENDERS - wrong accepted_lender_id or offer of that lender wasn't accepted");
                return res.status(http.statuses.OK).end();
            }
        } catch (error) {
