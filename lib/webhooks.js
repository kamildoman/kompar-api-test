...
const task_queue = require('./task_queue/task_queue');
const runners = require('./task_queue/runners');
...
exports.sendApplicationToLenders = async function(req, res, db, test) {
    ...
    if (client_id != null && application_id != null && token != null && token === auth(test)) {
        try {
            ...
            } else if (application.sentViaWebhook != null && application.sentViaWebhook && lender_id == null) {
                ...
                await (async function () {
                    for await (const webhook of webhooks) {
                        ...
                        if (is_clients_excluded.length === 0) {
                            if (lender_id == null || lender_id === clientsResponse.partner_id) {
                                if (clientsResponse != null && clientsResponse.delay != null) {
                                    await task_queue.queueTask(db, task_queue.type.delay, config.time.toCorrectZone(Date.now()) + clientsResponse.delay.time, {
                    ...
                    try {
                                    ...
                                    arrayApi.push(
                                        {
                                            ...
                                            "error": true,
                                            "type": "Application",
                                            "message": "Not confirmed of receiving application",
                                            "date": config.time.toCorrectZone(Date.now())
                                        }
                                    )
                                }
                        ...
                        if (lender_id != null) {
                            push["applications.$[replyapplication].webhooks"] = {"$each": shortSent(sents) };
                        } else {
                            set["applications.$[replyapplication].webhooks"] = shortSent(sents);
                        }
...
function encrypt(application, key) {
    ...
    const newestReport = reports[reports.length - 1].report;
    const body = {
        "application_id": application.id,
        "name": application.name,
        "company_id": application.company_id,
        "company_name": application.company_name,
        "company_type": newestReport.company_type,
        "company_revenue": parseFloat(application.company_revenue),
        "company_age": parseFloat(newestReport.years_business),
...
async function timeout(i, sent, times, application, db, test, client_id) {
    if (sent[i]["current"] < times.length) {
                ...
                .then(async res => {
                    if (res.status === 200) {
                        ...
                        await db.collection(config.DB.names.client).updateOne(
                            ...
                            {   
                                "$set": {
                                    "applications.$[replyapplication].webhooks.$[replywebhook].confirmed": true
                                },
                                "$push": {
                                    "applications.$[replyapplication].api":   {
                                        ...
                                        "error": false,
                                        "type": "Application",
                                        "message": "Confirmed of receiving app (" + sent[i]["current"] + " attempts)",
                                        "date": config.time.toCorrectZone(Date.now())
                                    }
                                }
                            },
                            { 
                                "arrayFilters": [
                                    { "replyapplication.id": application.id },
                                    { 'replywebhook._id': sent[i].webhook._id }
                                ]
                            }
                        );
                    } else {
        ...
        await db.collection(config.DB.names.client).updateOne(
            ...
            {   
                "$set": {
                    "applications.$[replyapplication].webhooks.$[replywebhook].after_resend_not_confirmed": true
                },
                "$push": {
                    "applications.$[replyapplication].api":   {
                        "lender_id": clientPartner.partner_id,
                    ...
                    for await (const webhook of webhooks) {
                        ...
                        if (clientsResponse.partner_id === accepted_lender_id.toString()) {
                            if (application.offer_lender_id.toString() === accepted_lender_id.toString()) {
                                ...
                                let body = null;
                                if (clientsResponse.encryptDecision) {
                                ...
                                try {
                                    ...
                                    if (webhookResponse.status === 200) {
                                        ...
                                        sents.push({ "webhook": webhook, "confirmed": false, "after_resend_not_confirmed": false, "proxy": clientsResponse.proxy, "encryptDecision": clientsResponse.encryptDecision });
                                    }
                                } catch(error) {
...
runners.addTaskRunner(task_queue.type.delay, async (req, res, db, variables, test, websockets) => {
    console.log("DELAY");
    ...
    const requestOptions = {
        ...
        body: variables.body,
        headers: {
            "Authorization": "token " + variables.webhook.auth,
            "Content-Type": "application/json"
        },
        timeout: webhooksConfig.timeout(test)
    };
    if (variables.proxy != null && variables.proxy === true) {
        requestOptions["agent"] = new HttpsProxyAgent(process.env.FIXIE_URL);
    }
    const webhookResponse = await fetch(variables.webhook.url, requestOptions);
    const sents = [];
    if (webhookResponse.status === 200) {
        sents.push({
                ...
                arrayApi.push(
                    {
                        ...
                        "error": false,
                        "type": "Application",
                        "message": "Correctly sent (after delay).",
                        "date": config.time.toCorrectZone(Date.now())
                    }
                )
            } else {
                arrayApi.push(
                    {
                        ...
                        "error": true,
                        "type": "Application",
                        "message": "Not confirmed of receiving application (delay)",
                        "date": config.time.toCorrectZone(Date.now())
                    }
                )
            }
    ...
    await db.collection(config.DB.names.client).updateOne(
        {
            ...
            "applications.id": variables.application_id
        },
        {
            "$push": {
        ...
        {
            "arrayFilters": [{'replyapplication.id': variables.application_id}]
        }
    );
    loop(variables.application, sents, db, test, variables.client_id);
})