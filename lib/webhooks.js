                    ...
                    const promise = fetch(webhook.url,
                        ...
                        .then(response => {
                            if (response.status === 200) {
                                sent.push({ "client_id": webhook.client_id, "confirmed": true });
                            } else {
                                sent.push({ "client_id": webhook.client_id, "confirmed": false });
                            }
                        }).catch(error => {
...
async function timeout(i, sent, times, application, db) {
    if (sent[i]["current"] < times.length) {
                ...
                .then(async res => {
                    if (res.status === 200) {
                        ...
                        await db.collection(config.DB.names.applications).updateOne(
                            { 
                                id: application.id,
                                "webhooks.client_id": sent[i].webhook.client_id
                            }, {
                            $set: {
        ...
        await db.collection(config.DB.names.applications).updateOne(
            { 
                id: application.id,
                "webhooks.client_id": sent[i].client_id
            }, {
            $set: {
...
exports.sendDecisionToLenders = async function(req, res, db) {
    ...
    if (id != null && lender_id != null) {
        try {
            ...
            logger.error(`WEBHOOKS - SEND DECISION TO LENDERS - MONGO error: ${error}`);
            return res.status(http.statuses.INTERNAL_SERVER_ERROR).end();
        }
    }
