        ...
        try {
            ...
            if (application == null) {
                ...
                webhooks.forEach(async function(webhook) {
                    const clients = await db.collection(config.DB.names.clients).findOne({
                        "id": webhook.client_id
                    })
                    console.log(additional_filtered_lenders)
                    console.log(clients)
                    const is_clients_excluded = additional_filtered_lenders.filter(a => a.id === parseInt(clients.partner_id));
