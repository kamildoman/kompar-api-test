...
exports.sendApplicationToLenders = async function(req, res, db, test) {
    ...
    return res.status(http.statuses.OK).send({ "success": false, "message": "You have to provide application id." });
}
            ...
            if (!application) {
                ...
                return Promise.all(promises)
                    .then(async () => {
                        ...
                        loopDecision(application, sent, db, test);
...
async function loopDecision(application, sent, db, test) {
    var times = [
        30000,    // 30000       30 seconds
        120000,   // 120000      2 minutes
        900000,   // 900000      15 minutes
    ...
    for (var i = 0; i < sent.length; i++) {
        sent[i]["current"] = 0;
        if (!sent[i].confirmed) {
            timeoutDecision(i, sent, times, application, db, test);
        }
    }
}

async function timeoutDecision(i, sent, times, application, db, test) {
    console.log(sent);
    if (sent[i]["current"] < times.length) {
        setTimeout(async function() {
