...
async function loop(application, sent, db) {
    const times = [
        ...
        5000,5000,5000//12000,    // 120000      2 minutes
...
async function timeout(i, sent, times, application, db) {
    if (sent[i]["current"] < times.length) {
            ...
            fetch(sent[i].webhook.url,
                ...
                .then(async res => {
                    if (res.status === 200) {
                        ...
                        await db.collection(config.DB.names.applications).updateOne({ id: application.id }, {
                            $set: {
                                webhooks: sent
                            }
                        });
        ...
        sent[i]["after_resend_not_confirmed"] = true;
        await db.collection(config.DB.names.applications).updateOne({ id: application.id }, {
            $set: {
                webhooks: sent
            }
